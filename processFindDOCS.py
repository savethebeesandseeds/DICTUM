...
# ---------
def identifyByFilename(filename, sentenciasData):
    for d_ in sentenciasData:
        ...
        # print("\t",flags)
        if(all(flags)):
            d_['path'].append(filename)
            print("\t POSITIVE: (byFilename) {}".format(filename))
def identifyByFilecontent(filename, page_content, sentenciasData):
    for d_ in sentenciasData:
        idfiers = []
...
if __name__ == "__main__":
    ...
    for file_ in [os.path.join(dirpath,filename) for dirpath, _, filenames in os.walk(processFolder) for filename in filenames if filename.endswith('.pdf')]:
        ...
        identifyByFilename(filename_, sentenciasData)
        if(byContent):
            identifyByFilecontent(filename_, page_content_, sentenciasData)
    # ------- SAVE ------
